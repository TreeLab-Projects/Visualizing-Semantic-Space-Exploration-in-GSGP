...
int main(int argc, char **argv){
    ...
    if (!trainFile_s.empty() && testFile_s.empty()){
        ...
        int index =0;
        cudaDeviceSynchronize();
        saveVisualSemantics(logPath, uDataTrainTarget, uSemanticTrainCases, 
            uSemanticRandomTrees, uSemanticTrainCasesNew, config.populationSize,
            outputNameFiles, nrow, 0, 1,0,0);
        saveVisualSemantics(logPath, uDataTrainTarget, uSemanticTrainCases, 
            uSemanticRandomTrees, uSemanticTrainCasesNew, config.populationSize,
            outputNameFiles, nrow, 0, 2,0,0);
        saveVisualSemantics(logPath, uDataTrainTarget, uSemanticTrainCases, 
            uSemanticRandomTrees, uSemanticTrainCasesNew, config.populationSize,
            outputNameFiles, nrow, 0, 3,0,0);
        ...
        for ( int generation=1; generation<=config.numberGenerations; generation++){
            ...
            saveVisualSemantics(logPath, uDataTrainTarget, uSemanticTrainCases, 
                uSemanticRandomTrees, uSemanticTrainCasesNew, config.populationSize,
                outputNameFiles, nrow, 0, 4,generation,0);
        ...
        saveVisualSemantics(logPath, uDataTrainTarget, uSemanticTrainCases, 
            uSemanticRandomTrees, uSemanticTrainCasesNew, config.populationSize,
            outputNameFiles, nrow, indexBestIndividual, 5,0,0);
        ...
        // Add file to save prediction with data test
        ...
        //variables to data test
        cudaMallocManaged(&uDataTest, sizeMemDataTest); 
        cudaMallocManaged(&uDataTestTarget, sizeof(float)*nrowTest);
        cudaMallocManaged(&uFitTest, sizeMemIndividuals);
        cudaMallocManaged(&uSemanticTestCases,sizeMemSemanticTest);
        cudaMallocManaged(&uSemanticTestRandomTrees,sizeMemSemanticTest);
        //variables to data test
        ...
        /*!< invokes the GPU to interpret the initial population with data test*/
        computeSemantics<<< gridSize, blockSize >>>(dInitialPopulation, uSemanticTestCases, individualLength, uDataTest, nrowTest, nvar, uPushGenes, uStackInd,config.protected_division);
        
        /*!< invokes the GPU to interpret the random trees with data test*/
        computeSemantics<<< gridSize, blockSize >>>(dRandomTrees, uSemanticTestRandomTrees, individualLength, uDataTest, nrowTest, nvar, uPushGenes, uStackInd,config.protected_division);
        ...
        if(config.errorFunction==0){
            fitTraining << 0 << "," <<uFit[indexBestIndividual]<<endl;
            fitTest     << 0 << "," <<uFitTest[indexBestIndividual]<<endl;    
        }else if (config.errorFunction==1){
            fitR2 = uFit[indexBestIndividual] - (-4000*(-1));
            fitTraining << 0 << "," << fitR2 <<endl;
        }
        ...
        cudaMallocManaged(&uSemanticTrainCasesNew,sizeMemSemanticTrain);
        cudaMallocManaged(&uFitNew, sizeMemPopulation);
        cudaMallocManaged(&uSemanticTestCasesNew,sizeMemSemanticTest);
        cudaMallocManaged(&uFitTestNew, sizeMemPopulation);
        ...
        // save visual semantic with data test
        saveVisualSemantics(logPath, uDataTestTarget, uSemanticTestCases, 
            uSemanticTestRandomTrees, uSemanticTestCasesNew, config.populationSize,
            outputNameFiles, nrowTest, 0, 1,0,1);
        saveVisualSemantics(logPath, uDataTestTarget, uSemanticTestCases, 
            uSemanticTestRandomTrees, uSemanticTestCasesNew, config.populationSize,
            outputNameFiles, nrowTest, 0, 2,0,1);
        saveVisualSemantics(logPath, uDataTestTarget, uSemanticTestCases, 
            uSemanticTestRandomTrees, uSemanticTestCasesNew, config.populationSize,
            outputNameFiles, nrowTest, 0, 3,0,1);
        /*!< main GSGP cycle*/
        for ( int generation=1; generation<=config.numberGenerations; generation++){
            ...
            if(config.oms==0){
                ...
                cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, geometricSemanticMutation, 0, sizeElementsSemanticTest);
                gridSize = (sizeElementsSemanticTest + blockSize - 1) / blockSize;
                
                if((gridSize*blockSize)>sizeElementsSemanticTest){
                    blockSize = 32;
                    gridSize = (sizeElementsSemanticTest + blockSize - 1) / blockSize;
                }else if((gridSize*blockSize)>sizeElementsSemanticTest){
                    blockSize = 16;
                    gridSize = (sizeElementsSemanticTest + blockSize - 1) / blockSize;
                }
                // printf("%i %i\n", gridSize,blockSize);
                geometricSemanticMutation<<< gridSize, blockSize >>>(uSemanticTestCases, uSemanticTestRandomTrees,uSemanticTestCasesNew,
                    config.populationSize, nrowTest, sizeElementsSemanticTest, generation, indexRandomTrees, vectorTraces, index, mutationStep, config.sigmoid, config.normalize);
                ...
                geometricSemanticMutation<<< gridSize, blockSize >>>(uSemanticTestCases, uSemanticTestRandomTrees,uSemanticTestCasesNew,
                    config.populationSize, nrowTest, sizeElementsSemanticTest, generation, indexRandomTrees, vectorTraces, index, oms, config.sigmoid, config.normalize);   
            ...
            if(config.errorFunction==0){
                ...
                computeError<<< gridSize,blockSize>>>(uSemanticTestCasesNew, uDataTestTarget, uFitTestNew, nrowTest);
                // printf("DEspues de ComputeError %i %i\n ",gridSize,blockSize );
            ...
            saveVisualSemantics(logPath, uDataTrainTarget, uSemanticTrainCases, 
                uSemanticRandomTrees, uSemanticTrainCasesNew, config.populationSize,
                outputNameFiles, nrow, 0, 4,generation,0);
            // save semantic of ofspring
            saveVisualSemantics(logPath, uDataTestTarget, uSemanticTestCases, 
                uSemanticTestRandomTrees, uSemanticTestCasesNew, config.populationSize,
                outputNameFiles, nrowTest, 0, 4,generation,1);
            ...
            if(config.errorFunction==0){
                fitTraining << generation << "," <<uFit[indexBestIndividual]<<endl;    
                fitTest << generation << ","<<uFitTest[indexBestIndividual]<<endl;
            }if (config.errorFunction==1){
                fitR2=0;
                fitR2 = uFit[indexBestIndividual] - (-4000*(-1));
                fitTraining << generation << "," << fitR2 <<endl;    
        ...
        saveVisualSemantics(logPath, uDataTrainTarget, uSemanticTrainCases, 
            uSemanticRandomTrees, uSemanticTrainCasesNew, config.populationSize,
            outputNameFiles, nrow, indexBestIndividual, 5,0,0);
        // save semantic of best
        saveVisualSemantics(logPath, uDataTestTarget, uSemanticTestCases, 
            uSemanticTestRandomTrees, uSemanticTestCasesNew, config.populationSize,
            outputNameFiles, nrowTest, indexBestIndividual, 5,0,1);
