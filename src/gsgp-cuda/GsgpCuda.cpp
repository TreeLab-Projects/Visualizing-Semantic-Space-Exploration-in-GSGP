...
__global__ void diferenceRt(int nrow, int populationSize ,
float *rtSemantics, float *indexRts, float *rtsDifference, int sigmoid, int normalize){
  const unsigned int tid = threadIdx.x+blockIdx.x*blockDim.x;
  int nSeed = (tid/nrow);
...
__global__ void computeNormalizeSemantic(float *rtSemantics, float *dataNormalize, int nrow, int min_max, int bandera){
 ...
 if (min_max==0) {
  ...
  for(int i=0; i<nrow; i++) {
    ...
    // if (tid==0) {
    //   printf("temporales %f %f \n", tmp, tmp2);
    // }
    for(int i=0; i<nrow; i++) {
      // if (i<10 && tid==0) {
      //   printf("semantica %f \n",rtSemantics[tid*nrow+i]);
      // }
      // rtSemantics[tid*nrow+i] = ((rtSemantics[tid*nrow+i]) - dataNormalize[tid*4])/dataNormalize[tid*4+1];  
      // rtSemantics[tid*nrow+i] = ((rtSemantics[tid*nrow+i]) - tmp)/tmp2;  
      rtSemantics[tid*nrow+i] = (((rtSemantics[tid*nrow+i]) - tmp)/(tmp2));  
      // if (i<10&& tid==0) {
      //   printf("semantica NORM%f \n",rtSemantics[tid*nrow+i]);
      // }
    }
    if (bandera==1) {
      float max=rtSemantics[tid*nrow];
...
void saveVisualSemantics(std::string path, float *target, float *semanticParents, 
  float *semanticRts, float *semanticOffsprings, int sizePop,
  std::string outputNameFiles, int nrow, int best, int etiqueta, int g, int dataType){


  if(dataType==0){
    outputNameFiles = outputNameFiles + "_visual_semantic.csv";
    outputNameFiles = path + outputNameFiles;
    std::ofstream outData(outputNameFiles,std::ios::app);
    ...
    if (etiqueta == 1) {
      ...
      outData << ",-1\n";
        ...
        // Semántica de la población
        // archivo << "- Semántica de la población (cada renglón es un individuo)\t\t\t\t\t2\n";
        for (int p=0;p<sizePop;p++) {
          for (int j =0;j<nrow;j++) {
            outData << semanticParents[p*nrow+j] << " ";
          }
          outData << ",-2\n";
        }
        // outData << "\n";
    } else if (etiqueta == 3) {
        // Semántica de los Random Trees
        // archivo << "- Semántica de los Random Trees\t\t\t\t\t\t\t\t\t\t3\n";
        for (int p=0;p<sizePop;p++) {
          for (int j =0;j<nrow;j++) {
              outData << semanticRts[p*nrow+j] << " ";
          }
          outData << ",-3\n";
        }
        // outData << "\n";
    } else if (etiqueta == 4) {
        // Semántica de cada población en cada generación
        // archivo << "- Semántica de cada población en cada generación\t\t\t\t\t\t\t\t4\n";
        for (int p=0;p<sizePop;p++) {
          for (int j =0;j<nrow;j++) {
            outData << semanticOffsprings[p*nrow+j] << " ";
          }
          // << "\",";
          outData << "," <<3+g <<"\n";
        }
        ...
        // Semántica de la mejor solución
        // archivo << "- Semántica de la mejor solución\t\t\t\t\t\t\t\t\t\t5\n";
        for (int j =0;j<nrow;j++) {
          outData << semanticParents[best*nrow+j] << " ";
        }
        outData << ",-5\n";
        // outData << "\n";
    } else {
        std::cerr << "Etiqueta inválida." << std::endl;
    }

    outData.close();

  }else{
    outputNameFiles = outputNameFiles + "_visual_semantic_test.csv";
    outputNameFiles = path + outputNameFiles;
    std::ofstream outData(outputNameFiles,std::ios::app);
